...
load("IgJtrim")    # 7-mer sequences extracted from human Ig J region (CDR3) (NCBI)
load("pepbioinfo") # peptide biological information defined in algoritmz.R
                   # contains pep, pepinfo, pepi and pepiused
load("vb10")       # the cleaned and normalized data    
load("ColnamesAndDiag") # patient codes and diagnosis
load("BloodGroupAndSex")
...
# Potential 7-mer idiotopes in the studied peptides
...
# Calculate the F statistic for all pairs of peptides to generate a dissimilarity matrix
ij=t(combn(length(pep),2))
J=cut(seq_along(ij[,1]), 1000, labels = F)
cl=makeCluster(10)                             # for parallelization pick your number of physical cores 
...
ajmFs=acast(x,X~Y)   # dissimilarity matrix
rownames(ajmFs)=pep
colnames(ajmFs)=pep

# Generate two groups of peptide reactivtiy pairs - 
# adjmdiag - (most probably) cross reactive and adjmsmpl - random
# and define suitable cut off
...
Gvs=set_vertex_attr(Gvs, "CV", 
          value =rowSds(vb010[names(V(Gvs)),])/rowMeans(vb010)[names(V(Gvs))])
...
chisq.posthoc.test(table(clqclqppi7$Clq2, 
cut(clqclqppi7$Idiotypes, c(-1,0.5,1.5,100), labels = F)), simulate.p.value=T)
...
res=t(sapply(1:2000,function(j){
  y1=sample(y0,50)
  yr1=clucri(vs10[y1,], dgnf)
  y2=sample(pep[!(pep %in% y0)],50)
  yr2=clucri(vs10[y2,], dgnf)
  return(cbind(yr1,yr2))
}))
colnames(res)=c("Peptides from Unique Cliques", "Random Peptide Sets")
vioplot(res, ylab="Diagnosis Clustering Criterion")

vioplot(cbind("Degree of in Unique Cliques"=log10(dGvs[unique(unlist(uniclq))]+0.5), 
        "Degree"=log10(sample(dGvs,length(unique(unlist(uniclq))))+0.5)))

# Comparison of "big" (>20) and "small" (<5) cliques
...
length(tbigclq)
length(tsmallclq)
sum(names(tbigclq) %in% vir)   # distribution of viral epitopes in big/small cliques
...
# Capacity of different size cliques to separate diagnostic groups 
# using clustering criterion
...
# Viral epitopes organize the big cliques
...
# Over representation of HTLV1 sequences in the big cliques
...
# Viral epitopes are found more oftren in many cliques than self sequences 
...
seqtoFASTA(bigclqpep, "bigclq.txt") # ... to MEME XSTREME

# Viral epitopes, clique sizes and blood groups
x=names(tbigclq)
y=names(tsmallclq)
...
# Clique size distribution and idiotopes
X=lapply(clqord, names)
X=melt(X)
...
# Bootstrap the distribution of the clustering criterion by size of data sample
# (dimensionality of the embedding space, corresponds to cliques sizes 3 -10)
...
# Diagnoses clustering criterion for subspaces defined by the 3-10 cliques
cl=makeCluster(15)
...
# Empirical distribution function of the bootstrapped distributions
crclcr=lapply(3:10,function(i) ecdf(bsx[[as.character(i)]]))
  
# Selecting cliques with a high probability to be diagnostic 
...
# Recursive feature elimination selects from the "positive" cliques peptides 
# sets with pretty good capacity to separate but this is just to know...
selFposclqp=Loolocal0(posclqpep,mx)
plotMDS(vs10[selFposclqp,], col=dgnf, pch=16)

# Adjacency matrix of the "positive" cliques...
...
# Comparison of cliques of cliques and the main properties
...
# Stats of clq2 induced subgraph of the reacitvity graph ----------------------
...
# PCA of Ags x Cliques of cliques
X=Clq2Ags_mx
...
# Proportion of viral epitopes as a function of clique size
